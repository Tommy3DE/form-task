...
const List = ({ colorList, setColorList }: ListProps) => {
  ...
  const sortedColorList = [...colorList].sort((colorA, colorB) => {
    const [rA, gA, bA] = getRGBValues(colorA);
    const [rB, gB, bB] = getRGBValues(colorB);
    if (rA !== rB) {
      return rB - rA;
    } else if (gA !== gB) {
      return gB - gA;
  ...
  return (
    <div>
      <ul className='goonline_list-ul'>
        {sortedColorList.map((color, index) => (
          <li key={index}>
            <div className='goonline_list-ul-content'>
              ...
              {!initialColors.includes(color) && (
                <button onClick={() => handleDelete(index, color)}>X</button>
              )}
            </div>
          </li>
        ))}
